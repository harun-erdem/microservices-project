...
docker build --force-rm -t "${IMAGE_TAG_CUSTOMERS_SERVER}" "${WORKSPACE}/spring-petclinic-customers-service"
...
docker build --force-rm -t "${IMAGE_TAG_GRAFANA_SERVER}" "${WORKSPACE}/docker/grafana"
docker build --force-rm -t "${IMAGE_TAG_PROMETHEUS_SERVER}" "${WORKSPACE}/docker/prometheus"